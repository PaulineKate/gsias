(function () {
    if (document.getElementById('casualPopupStyle')) return;
    const s = document.createElement('style');
    s.id = 'casualPopupStyle';
    s.textContent =
        '@keyframes casualFadeIn  { from { opacity: 0; transform: scale(0.96); }' +
                                   ' to   { opacity: 1; transform: scale(1);    } }' +
        '@keyframes casualFadeOut { from { opacity: 1; } to { opacity: 0; } }';
    document.head.appendChild(s);
})();

function showPopup(message, type) {
    const existing = document.getElementById('casualPopupOverlay');
    ...
    box.style.cssText =
        ...
        'animation:casualFadeIn 0.2s ease;';
    ...
    document.getElementById('casualPopupOkBtn').addEventListener('click', function () {
        overlay.remove();
    });
...
function previewFile(btn) {
    ...
    const pdfPath = 'Casual_Contract_files/' + refFolder + '.pdf';
...
(function () {
    const searchInput = document.getElementById('casualSearch');
    const tableBody   = document.getElementById('casualTableBody');
    const dataRows    = tableBody
        ? Array.from(tableBody.querySelectorAll('tr.casual-data-row'))
        : [];
